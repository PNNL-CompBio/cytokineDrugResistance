...
phosData <- querySynapseTable('syn22986341')%>%#syn24389738') %>%
  subset(!is.nan(LogRatio)) %>%
  mutate(Gene = unlist(Gene)) %>%
  mutate(site = unlist(site))

uncorrectedPhosData <-querySynapseTable('syn24389738') %>%
  subset(!is.nan(LogRatio)) %>%
  mutate(Gene = unlist(Gene)) %>%
  mutate(site = unlist(site))
...
ucPhosMat <-phospho_data_to_matrix(uncorrectedPhosData)
```
...
uncorrectedPhosData %>% filter(Batch == "Experiment 2",
                    TimePoint != 120) %>%
...
#pdf(file.path(output_dir, "Late_MOLM13_w_Parental_phospho_PCA.pdf"))
uncorrectedPhosData %>% filter(Batch == "Experiment 1",
                    CellType == "MOLM-13 Tr Resistant",
                    Treatment != "Trametinib Withdrawn") %>%
  plotAllData(colour='Treatment', fill='Treatment')+
...
setwd(output_dir)
KSEA_plots_output <- doAllKSEAplots(limma_DEA_results, x, output_dir = "./ksea_plots", p_cutoff=0.05)
#output results
write.table(KSEA_plots_output, file='lateOnly_KSEA.tsv',
            quote=FALSE, sep='\t', col.names = NA)
```
...
Here we see the heatmap of the kinase activity
...
limma_DEA_results <- list(Tram_vs_Parental_all = compare_samples(x,
                                                   "Trametinib",       "none"),
                MCP1_vs_Parental_all = compare_samples(x,
                                                   "MCP-1", "none"),
                TramMCP_vs_Parental_all = compare_samples(x,
                                                   "Trametinib+MCP-1",  "none"))
...
KSEA_plots_output <- doAllKSEAplots(limma_DEA_results, x, output_dir = "./ksea_plots")
#output results
write.table(KSEA_plots_output, file='Molm3_all_KSEA.tsv',
            quote=FALSE, sep='\t', col.names = NA)
