...
plotKinDat<-function(kindat,sig.kin=NULL,prefix='all'){
  ...
  sampAts<-phosData%>%
    dplyr::select(sample,TimePoint,Treatment)%>%
    distinct()%>%
    tibble::remove_rownames()%>%
  ...
  pheatmap(mat,cellwidth = 8,cellheight=8,clustering_distance_cols = 'correlation',
          ...
          annotation_colors=sampCols,annotation_col=sampAts) 
...
phosData%>%
  ...
  ggtitle("Phosphoproteomics Experiment 1")
...
m13Phos<-list(Tram_5min=limmaTwoFactorDEAnalysis(phosMat,    
 ...
 # print(sig.kin)
  ...
  plotKinDat(subset(kindat,
             ...
             sig.kin$Kinase.Gene,'exp1_singleTimeCourse')
...
#  #lateHeatmap<-kseaZscoreHeatmap(list(p3,p5),'earlyTreatmentKSEAzscoreHeatmap.pdf')
 ...
 # print(sig.kin)
  ...
  plotKinDat(subset(kindat,Sample%in%subset(summary,CellType=='MOLM-13')$sample),
             sig.kin$Kinase.Gene,'exp1_comboTimeCourse')
...
phosMat <- earlyPhosMat
summary <-earlySummary
latePhos<-list(resist_vs_parental=limmaTwoFactorDEAnalysis(phosMat,
                        filter(summary,conditionName=='MOLM-13_0_none')$sample,
...
#  ggsave('earlyLatePhos.png',earlyLatePh,width=11,height=6)
  ...
  plotKinDat(subset(kindat,
             ...
             sig.kin$Kinase.Gene,'resistantCellLines')

```
These represent the late resistance samples


## Plot selected kinases

We have a list of kinases from Rucha that she wants to see. 

```{r kinase activity}

  kins=c('CDK1', 'CDK2', 'CDK4', 'CDK6', 'MAPK1', 'MAPK3', 'MAPK9', 'SRC', 'MTOR', 'JAK2', 'JAK3','TYK2','HCK','LCK')

 p<- kindat%>%dplyr::rename(sample='Sample')%>%left_join(rbind(earlySummary,lateSummary))%>%subset(Kinase%in%kins)%>%subset(!Treatment%in%c('Treatment Withdrawn',NA))%>%ggplot(aes(x=TimePoint,y=meanLFC,color=Treatment,shape=CellType))+geom_jitter()+facet_grid(CellType~Kinase)  
 #print(p)
...
tws<-subset(earlySummary,Treatment%in%c('Trametinib Withdrawn',NA))$sample
 ...
 kindat%>%
   ...
   plotKinDat(.,kins,prefix='selectedExp1')
 #ggsave('selectedKinaseActivity.pdf',p)
```
<!-- ### Network analysis -->
